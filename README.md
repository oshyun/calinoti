...
- 알림 배경색 설정(라이트·다크 테마별 HEX 지정) — One UI가 커스텀 알림 카드를 표준보다
  연하게 그려 기본 알림 카드와 밝기가 어긋나므로 카드 배경을 직접 그린다. 미지정 시 테마별
  기본값(One UI 실기기 카드색에서 추출: F4F4F4/515151), "기본값으로 지정" 버튼으로 복원
